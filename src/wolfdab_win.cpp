...
namespace {
constexpr const wchar_t* APP_VERSION=L"1.0.21";
...
constexpr int ID_PTY=134;
constexpr int ID_CHANNELS=135, ID_SCID=136;
constexpr int IDM_NEW=200, IDM_OPEN=201, IDM_SAVE=202, IDM_SAVE_AS=203, IDM_EXIT=204, IDM_ABOUT=205, IDM_REGISTER=206, IDM_UPDATE=207;
constexpr int ID_ACCENT=900;
constexpr UINT WM_UPDATE_RESULT=WM_APP+17;

struct Service { std::wstring label=L"Nuovo servizio"; std::wstring shortLabel=L"Nuovo"; std::wstring sid=L"0xE001"; unsigned scids=0; unsigned subch=1; std::wstring source=L""; unsigned bitrate=72; unsigned codec=1; unsigned sampling=48000; dab_eep_profile_t eep=DAB_EEP_3A; std::wstring dls=L"";std::wstring motFolder=L"";unsigned motInterval=10;unsigned pty=0;unsigned channels=2;
    Service()=default;
...
HWND wnd, list, statusbar, editService, editSid, editScids, editScidValue, editSubch, comboSource, editSource, editRate, comboCodec, comboSampling, comboEep, comboPty, comboChannels, editDls, editMotFolder, editMotInterval, motFolderLabel, motIntervalLabel, editEnsemble, editEid, editEcc, comboChannel, editGain, ampCheck, applyFormatButton;
HINSTANCE instance;
HFONT uiFont, titleFont;
HBRUSH windowBrush, accentBrush;
HANDLE txProcess, txStopEvent;
bool txStopping=false;
...
void ensureMotControls(){if(editMotFolder)return;motFolderLabel=CreateWindowW(L"STATIC",tr(L"Cartella immagini MOT",L"MOT image folder"),WS_CHILD|WS_VISIBLE,696,450,250,20,wnd,0,0,0);motIntervalLabel=CreateWindowW(L"STATIC",tr(L"Intervallo",L"Interval"),WS_CHILD|WS_VISIBLE,1184,450,72,20,wnd,0,0,0);editMotFolder=edit(ID_MOT_FOLDER,696,472,430);CreateWindowW(L"BUTTON",L"…",WS_CHILD|WS_VISIBLE,1134,472,37,24,wnd,(HMENU)ID_MOT_BROWSE,0,0);editMotInterval=edit(ID_MOT_INTERVAL,1184,472,48);createLabel(L"s",1237,475,20);SetWindowPos(ampCheck,nullptr,696,508,280,26,SWP_NOZORDER);SetWindowPos(GetDlgItem(wnd,ID_START),nullptr,696,540,160,42,SWP_NOZORDER);SetWindowPos(GetDlgItem(wnd,ID_STOP),nullptr,870,540,160,42,SWP_NOZORDER);SendMessageW(editMotFolder,WM_SETFONT,(WPARAM)uiFont,TRUE);SendMessageW(editMotInterval,WM_SETFONT,(WPARAM)uiFont,TRUE);SendMessageW(motFolderLabel,WM_SETFONT,(WPARAM)uiFont,TRUE);SendMessageW(motIntervalLabel,WM_SETFONT,(WPARAM)uiFont,TRUE);}
...
void saveSelected(bool refresh=true) {
    ...
    std::wstring scidText=text(editScidValue);wchar_t*scidEnd=nullptr;unsigned long scid=wcstoul(scidText.c_str(),&scidEnd,10);if(scidEnd&&scidEnd!=scidText.c_str()&&*scidEnd==0&&scid<=15)s.scids=(unsigned)scid;
    ...
    try{s.bitrate=(unsigned)std::stoul(text(editRate));}catch(...){s.bitrate=0;}s.codec=(unsigned)SendMessageW(comboCodec,CB_GETCURSEL,0,0);int sm=(int)SendMessageW(comboSampling,CB_GETCURSEL,0,0);s.sampling=sm==1?48000:(s.codec==3?24000:32000);int p=(int)SendMessageW(comboEep,CB_GETCURSEL,0,0);if(p>=0)s.eep=(dab_eep_profile_t)p;int pt=(int)SendMessageW(comboPty,CB_GETCURSEL,0,0);if(pt>=0)s.pty=(unsigned)pt;s.channels=SendMessageW(comboChannels,CB_GETCURSEL,0,0)==0?1u:2u;
...
void finishTx(){
    if(txProcess){CloseHandle(txProcess);txProcess=nullptr;}
    if(txStopEvent){CloseHandle(txStopEvent);txStopEvent=nullptr;}
    txStopping=false;
    EnableWindow(GetDlgItem(wnd,ID_START),TRUE);
    EnableWindow(GetDlgItem(wnd,ID_STOP),FALSE);
    status();
}
void stopTx(){
    if(!txProcess)return;
    if(!txRunning()){finishTx();return;}
    if(txStopEvent)SetEvent(txStopEvent);
    /* Never force-terminate the TX process: doing so can leave the USB
       streaming handle owned by libhackrf until the device is replugged. */
    txStopping=true;
    EnableWindow(GetDlgItem(wnd,ID_START),FALSE);
    EnableWindow(GetDlgItem(wnd,ID_STOP),FALSE);
    SendMessageW(statusbar,SB_SETTEXT,0,(LPARAM)tr(L"Arresto TX e rilascio HackRF…",L"Stopping TX and releasing HackRF…"));
}
void startTx(){
    saveSelected(false);
    if(txRunning())return;
    ...
    BOOL ok=CreateProcessW(exe,buf.data(),nullptr,nullptr,FALSE,CREATE_NO_WINDOW,nullptr,dir.c_str(),&si,&pi);if(!ok){CloseHandle(txStopEvent);txStopEvent=nullptr;MessageBoxW(wnd,tr(L"Impossibile avviare il motore TX.",L"Unable to start the TX engine."),L"WolfDAB",MB_ICONERROR);return;}CloseHandle(pi.hThread);txProcess=pi.hProcess;txStopping=false;EnableWindow(GetDlgItem(wnd,ID_START),FALSE);EnableWindow(GetDlgItem(wnd,ID_STOP),TRUE);SetTimer(wnd,1,500,nullptr);status();
}
struct UpdateResult{bool silent{},ok{},newer{};std::wstring version,url;};
std::string httpGet(const wchar_t*host,const wchar_t*path){std::string out;HINTERNET ses=WinHttpOpen(L"WolfDAB/1.0",WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY,WINHTTP_NO_PROXY_NAME,WINHTTP_NO_PROXY_BYPASS,0);if(!ses)return out;WinHttpSetTimeouts(ses,6000,6000,6000,6000);HINTERNET con=WinHttpConnect(ses,host,INTERNET_DEFAULT_HTTPS_PORT,0);HINTERNET req=con?WinHttpOpenRequest(con,L"GET",path,nullptr,WINHTTP_NO_REFERER,WINHTTP_DEFAULT_ACCEPT_TYPES,WINHTTP_FLAG_SECURE):nullptr;if(req&&WinHttpSendRequest(req,WINHTTP_NO_ADDITIONAL_HEADERS,0,nullptr,0,0,0)&&WinHttpReceiveResponse(req,nullptr)){DWORD status=0,z=sizeof(status);WinHttpQueryHeaders(req,WINHTTP_QUERY_STATUS_CODE|WINHTTP_QUERY_FLAG_NUMBER,nullptr,&status,&z,nullptr);if(status==200)for(;;){DWORD n=0;if(!WinHttpQueryDataAvailable(req,&n)||!n)break;size_t old=out.size();out.resize(old+n);DWORD got=0;if(!WinHttpReadData(req,out.data()+old,n,&got)){out.resize(old);break;}out.resize(old+got);}}if(req)WinHttpCloseHandle(req);if(con)WinHttpCloseHandle(con);WinHttpCloseHandle(ses);return out;}
std::string jsonValue(const std::string&j,const char*key){std::string mark="\""+std::string(key)+"\"";size_t p=j.find(mark);if(p==std::string::npos)return{};p=j.find(':',p+mark.size());if(p==std::string::npos)return{};p=j.find('"',p+1);if(p==std::string::npos)return{};size_t e=j.find('"',p+1);return e==std::string::npos?std::string():j.substr(p+1,e-p-1);}
std::vector<int> versionParts(const std::wstring&v){std::vector<int>r;size_t p=0;while(p<v.size()){while(p<v.size()&&!iswdigit(v[p]))++p;if(p>=v.size())break;size_t e=p;while(e<v.size()&&iswdigit(v[e]))++e;r.push_back(std::stoi(v.substr(p,e-p)));p=e;}return r;}
bool newerVersion(const std::wstring&a,const std::wstring&b){auto x=versionParts(a),y=versionParts(b);size_t n=std::max(x.size(),y.size());x.resize(n);y.resize(n);return x>y;}
void checkUpdates(bool silent){auto*arg=new UpdateResult;arg->silent=silent;HANDLE thread=CreateThread(nullptr,0,[](LPVOID p)->DWORD{auto*r=(UpdateResult*)p;std::string j=httpGet(L"www.freewaves.it",L"/WolfDAB_version.json");std::string v=jsonValue(j,"version"),u=jsonValue(j,"url");if(!v.empty()&&!u.empty()){r->ok=true;r->version.assign(v.begin(),v.end());r->url.assign(u.begin(),u.end());r->newer=newerVersion(r->version,APP_VERSION);}if(!PostMessageW(wnd,WM_UPDATE_RESULT,0,(LPARAM)r))delete r;return 0;},arg,0,nullptr);if(thread)CloseHandle(thread);else delete arg;}
const wchar_t* updateText(const wchar_t*it,const wchar_t*en,const wchar_t*de,const wchar_t*fr){return registrationLanguage==0?it:registrationLanguage==2?de:registrationLanguage==3?fr:en;}
void addMenus(){ HMENU bar=CreateMenu(),file=CreatePopupMenu(),help=CreatePopupMenu(); AppendMenuW(file,MF_STRING,IDM_NEW,tr(L"Nuova configurazione",L"New configuration"));AppendMenuW(file,MF_STRING,IDM_OPEN,tr(L"Apri…",L"Open…"));AppendMenuW(file,MF_STRING,IDM_SAVE,tr(L"Salva",L"Save"));AppendMenuW(file,MF_STRING,IDM_SAVE_AS,tr(L"Salva con nome…",L"Save as…"));AppendMenuW(file,MF_SEPARATOR,0,0);AppendMenuW(file,MF_STRING,IDM_REGISTER,tr(L"Registrazione",L"Registration"));AppendMenuW(file,MF_SEPARATOR,0,0);AppendMenuW(file,MF_STRING,IDM_EXIT,tr(L"Esci",L"Exit"));AppendMenuW(help,MF_STRING,IDM_UPDATE,tr(L"Controlla aggiornamenti",L"Check for updates"));AppendMenuW(help,MF_STRING,IDM_ABOUT,tr(L"Informazioni",L"About"));AppendMenuW(bar,MF_POPUP,(UINT_PTR)file,tr(L"File",L"File"));AppendMenuW(bar,MF_POPUP,(UINT_PTR)help,tr(L"Aiuto",L"Help"));SetMenu(wnd,bar); }
void createUi(){InitCommonControls();CreateWindowW(L"STATIC",L"",WS_CHILD|WS_VISIBLE,0,0,1280,5,wnd,(HMENU)ID_ACCENT,instance,0);HICON icon=(HICON)LoadImageW(instance,MAKEINTRESOURCEW(101),IMAGE_ICON,56,56,LR_DEFAULTCOLOR);HWND logo=CreateWindowW(L"STATIC",L"",WS_CHILD|WS_VISIBLE|SS_ICON,16,14,60,60,wnd,0,instance,0);SendMessageW(logo,STM_SETICON,(WPARAM)icon,0);HWND title=CreateWindowW(L"STATIC",L"WolfDAB",WS_CHILD|WS_VISIBLE,88,16,220,34,wnd,0,0,0);CreateWindowW(L"STATIC",tr(L"Multiplex DAB/DAB+ per HackRF One",L"DAB/DAB+ multiplex for HackRF One"),WS_CHILD|WS_VISIBLE,90,48,360,22,wnd,0,0,0);createLabel(tr(L"Servizi nel multiplex",L"Services in multiplex"),16,86,260);list=CreateWindowExW(WS_EX_CLIENTEDGE,WC_LISTVIEWW,L"",WS_CHILD|WS_VISIBLE|LVS_REPORT|LVS_SINGLESEL,16,110,650,210,wnd,0,0,0);ListView_SetExtendedListViewStyle(list,LVS_EX_FULLROWSELECT|LVS_EX_GRIDLINES|LVS_EX_DOUBLEBUFFER);const wchar_t*h[]={tr(L"Etichetta",L"Label"),L"Service ID",L"Bitrate",L"Codec",tr(L"Sampling",L"Sampling"),L"EEP",L"CU",L"PTY"};int widths[]={130,76,68,92,66,72,40,50};for(int i=0;i<8;i++){LVCOLUMNW c{};c.mask=LVCF_TEXT|LVCF_WIDTH;c.pszText=(LPWSTR)h[i];c.cx=widths[i];ListView_InsertColumn(list,i,&c);}CreateWindowW(L"BUTTON",tr(L"Aggiungi",L"Add"),WS_CHILD|WS_VISIBLE,16,334,100,32,wnd,(HMENU)ID_ADD,0,0);CreateWindowW(L"BUTTON",tr(L"Rimuovi",L"Remove"),WS_CHILD|WS_VISIBLE,126,334,100,32,wnd,(HMENU)ID_REMOVE,0,0);CreateWindowW(L"BUTTON",L"Italiano / English",WS_CHILD|WS_VISIBLE,520,334,146,32,wnd,(HMENU)ID_LANG,0,0);
createLabel(tr(L"Servizio selezionato",L"Selected service"),696,86,300);createLabel(tr(L"Etichetta",L"Label"),696,116);editService=edit(ID_SERVICE,696,138,566);createLabel(L"Service ID",696,172);editSid=edit(ID_SID,696,194,150);createLabel(tr(L"Tipo sorgente",L"Source type"),696,228);comboSource=CreateWindowW(WC_COMBOBOXW,L"",WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST,696,250,566,180,wnd,(HMENU)ID_SOURCE,0,0);for(auto*s:{tr(L"Dispositivo / WASAPI",L"Device / WASAPI"),tr(L"Generatore di tono",L"Tone generator"),tr(L"File audio locale",L"Local audio file"),L"Stream HTTP / HTTPS"})SendMessageW(comboSource,CB_ADDSTRING,0,(LPARAM)s);createLabel(tr(L"Dispositivo, tono o indirizzo",L"Device, tone or address"),696,284,320);editSource=edit(128,696,306,521);CreateWindowW(L"BUTTON",L"…",WS_CHILD|WS_VISIBLE,1225,306,37,24,wnd,(HMENU)ID_BROWSE,0,0);createLabel(L"Bitrate",696,340,70);editRate=edit(ID_BITRATE,696,362,70);createLabel(L"CODEC",777,340,120);comboCodec=CreateWindowW(WC_COMBOBOXW,L"",WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST,777,362,120,160,wnd,(HMENU)ID_CODEC,0,0);for(auto*s:{L"AAC-LC",L"HE-AAC v1",L"HE-AAC v2"})SendMessageW(comboCodec,CB_ADDSTRING,0,(LPARAM)s);createLabel(L"Sampling",908,340,86);comboSampling=CreateWindowW(WC_COMBOBOXW,L"",WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST,908,362,86,100,wnd,(HMENU)ID_SAMPLING,0,0);SendMessageW(comboSampling,CB_ADDSTRING,0,(LPARAM)L"32 kHz");SendMessageW(comboSampling,CB_ADDSTRING,0,(LPARAM)L"48 kHz");createLabel(L"EEP",1005,340,90);comboEep=CreateWindowW(WC_COMBOBOXW,L"",WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST,1005,362,90,180,wnd,(HMENU)ID_EEP,0,0);createLabel(L"PTY",1106,340,156);comboPty=CreateWindowW(WC_COMBOBOXW,L"",WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST|WS_VSCROLL,1106,362,156,320,wnd,(HMENU)ID_PTY,0,0);fillPtys();createLabel(L"DLS",696,396);editDls=edit(ID_DLS,696,418,566);
createLabel(tr(L"Ensemble",L"Ensemble"),16,480);createLabel(tr(L"Etichetta",L"Label"),16,510);editEnsemble=edit(ID_ENSEMBLE,16,532,220);createLabel(L"Ensemble ID",250,510);editEid=edit(ID_EID,250,532,110);createLabel(L"ECC",375,510);editEcc=edit(ID_ECC,375,532,70);createLabel(tr(L"Blocco",L"Block"),460,510);comboChannel=CreateWindowW(WC_COMBOBOXW,L"",WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST,460,532,90,180,wnd,(HMENU)ID_CHANNEL,0,0);for(int n=5;n<=13;n++)for(wchar_t l:L"ABCD"){wchar_t q[8];swprintf(q,8,L"%d%c",n,l);SendMessageW(comboChannel,CB_ADDSTRING,0,(LPARAM)q);}SendMessageW(comboChannel,CB_SETCURSEL,-1,0);createLabel(tr(L"Gain TX",L"TX gain"),565,510);editGain=edit(ID_GAIN,565,532,70);ampCheck=CreateWindowW(L"BUTTON",tr(L"Amplificatore ON",L"Amplifier ON"),WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX,666,474,280,26,wnd,(HMENU)ID_AMP,0,0);SendMessageW(ampCheck,BM_SETCHECK,BST_UNCHECKED,0);CreateWindowW(L"BUTTON",tr(L"Avvia TX",L"Start TX"),WS_CHILD|WS_VISIBLE|BS_DEFPUSHBUTTON,666,510,160,42,wnd,(HMENU)ID_START,0,0);CreateWindowW(L"BUTTON",tr(L"Ferma TX",L"Stop TX"),WS_CHILD|WS_VISIBLE,840,510,160,42,wnd,(HMENU)ID_STOP,0,0);statusbar=CreateWindowW(STATUSCLASSNAMEW,L"",WS_CHILD|WS_VISIBLE,0,0,0,0,wnd,0,0,0);uiFont=CreateFontW(-16,0,0,0,FW_NORMAL,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,DEFAULT_PITCH,L"Segoe UI");titleFont=CreateFontW(-26,0,0,0,FW_SEMIBOLD,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,DEFAULT_PITCH,L"Segoe UI");EnumChildWindows(wnd,applyFont,(LPARAM)uiFont);SendMessageW(title,WM_SETFONT,(WPARAM)titleFont,TRUE);fillProfiles();populateList(); }
...
LRESULT CALLBACK proc(HWND h,UINT m,WPARAM w,LPARAM l){
 switch(m){
 case WM_CREATE:wnd=h;windowBrush=CreateSolidBrush(RGB(248,249,251));accentBrush=CreateSolidBrush(RGB(158,235,55));{BOOL dark=FALSE;DwmSetWindowAttribute(h,20,&dark,sizeof(dark));DWORD corner=2;DwmSetWindowAttribute(h,33,&corner,sizeof(corner));}addMenus();createUi();SendMessageW(comboCodec,CB_ADDSTRING,0,(LPARAM)L"DAB MP2");
#if !defined(WOLFDAB_OWNER_BUILD)
if(!wolfdab_license::is_registered()&&!wolfdab_license::show_registration(h,registrationLanguage)){registrationFailed=true;PostMessageW(h,WM_CLOSE,0,0);return 0;}
#endif
if(startupConfig.empty())startupConfig=readLastConfig();if(!startupConfig.empty()){configPath=startupConfig;loadConfig(configPath);}refreshLanguage();EnableWindow(GetDlgItem(h,ID_STOP),FALSE);checkUpdates(true);return 0;
 ...
 case WM_COMMAND:{int id=LOWORD(w),code=HIWORD(w);if(code==CBN_SELCHANGE&&id==ID_CODEC){unsigned co=(unsigned)SendMessageW(comboCodec,CB_GETCURSEL,0,0);fillSampling(co,48000);}if(code==EN_CHANGE&&(id==ID_SCIDS||id==ID_SCID||id==ID_SUBCH)){int i=selectedService;if(i>=0&&(size_t)i<services.size()){std::wstring v=text((HWND)l);if(id==ID_SCIDS){services[(size_t)i].shortLabel=v.substr(0,8);}else{wchar_t*end=nullptr;unsigned long n=wcstoul(v.c_str(),&end,10);if(end&&*end==0){if(id==ID_SCID&&n<=15)services[(size_t)i].scids=(unsigned)n;else if(id==ID_SUBCH&&n<=63)services[(size_t)i].subch=(unsigned)n;}}}}if(code==EN_CHANGE&&id==ID_BITRATE&&selectedService>=0&&(size_t)selectedService<services.size()){std::wstring v=text(editRate);wchar_t*end=nullptr;unsigned long n=wcstoul(v.c_str(),&end,10);if(end&&end!=v.c_str()&&*end==0){services[(size_t)selectedService].bitrate=(unsigned)n;int keep=selectedService;populateList();selectedService=keep;ListView_SetItemState(list,keep,LVIS_SELECTED,LVIS_SELECTED);status();}}if((code==EN_KILLFOCUS&&(id==ID_SERVICE||id==ID_SID||id==ID_SOURCE_VALUE||id==ID_BITRATE||id==ID_DLS||id==ID_SCIDS||id==ID_SCID||id==ID_SUBCH||id==ID_MOT_FOLDER||id==ID_MOT_INTERVAL))||(code==CBN_SELCHANGE&&(id==ID_SOURCE||id==ID_CODEC||id==ID_SAMPLING||id==ID_EEP||id==ID_PTY)))saveSelected(false);switch(id){
  ...
  case IDM_NEW:services.clear();selectedService=-1;configPath.clear();setText(editService,L"");setText(editSid,L"");setText(editScids,L"");setText(editSubch,L"");setText(editSource,L"");setText(editRate,L"");setText(editDls,L"");setText(editMotFolder,L"");setText(editMotInterval,L"");setText(editEnsemble,L"");setText(editEid,L"");setText(editEcc,L"");setText(editGain,L"");SendMessageW(comboChannel,CB_SETCURSEL,-1,0);SendMessageW(ampCheck,BM_SETCHECK,BST_UNCHECKED,0);populateList();break;
  ...
  case IDM_EXIT:SendMessageW(h,WM_CLOSE,0,0);break;case IDM_ABOUT:info();break;case IDM_UPDATE:checkUpdates(false);break;case IDM_REGISTER:if(wolfdab_license::is_registered())MessageBoxW(h,tr(L"WolfDAB è registrato su questo PC.",L"WolfDAB is registered on this PC."),L"WolfDAB",MB_OK|MB_ICONINFORMATION);else wolfdab_license::show_registration(h,registrationLanguage);break;case ID_START:startTx();break;case ID_STOP:stopTx();break;}break;}
 case WM_CLOSE:if(txProcess&&txRunning()){stopTx();return 0;}finishTx();DestroyWindow(h);return 0;
...
int WINAPI wWinMain(HINSTANCE i,HINSTANCE, PWSTR cmd,int){instance=i;WORD lang=PRIMARYLANGID(GetUserDefaultUILanguage());registrationLanguage=lang==LANG_ITALIAN?0:lang==LANG_GERMAN?2:lang==LANG_FRENCH?3:1;italian=registrationLanguage==0;if(cmd&&wcscmp(cmd,L"--license-check")==0){
#if defined(WOLFDAB_OWNER_BUILD)
return 0;
#else
return wolfdab_license::is_registered()?0:10;
#endif
}if(cmd&&wcsncmp(cmd,L"--config",8)==0){const wchar_t*p=cmd+8;while(*p==L' ')++p;if(*p==L'\"'){++p;const wchar_t*e=wcschr(p,L'\"');startupConfig.assign(p,e?e:p+wcslen(p));}else startupConfig=p;}WNDCLASSW c{};c.hInstance=i;c.lpszClassName=L"WolfDAB";c.lpfnWndProc=proc;c.hCursor=LoadCursor(nullptr,IDC_ARROW);c.hIcon=LoadIconW(i,MAKEINTRESOURCEW(101));c.hbrBackground=(HBRUSH)(COLOR_WINDOW+1);if(!RegisterClassW(&c)&&GetLastError()!=ERROR_CLASS_ALREADY_EXISTS)return 1;constexpr DWORD style=WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX|WS_VISIBLE;HWND h=CreateWindowW(c.lpszClassName,L"WolfDAB 1.0.19",style,50,35,1300,700,nullptr,nullptr,i,nullptr);if(!h)return 2;MSG m;while(GetMessageW(&m,nullptr,0,0)){TranslateMessage(&m);DispatchMessageW(&m);}return registrationFailed?10:0;}
