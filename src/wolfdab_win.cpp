...
HWND wnd, list, statusbar, editService, editSid, editScids, editSubch, comboSource, editSource, editRate, comboCodec, comboSampling, comboEep, editDls, editEnsemble, editEid, editEcc, comboChannel, editGain, ampCheck, applyFormatButton;
HINSTANCE instance;
...
void loadSelected();
void ensureBulkControl(){if(!applyFormatButton)applyFormatButton=CreateWindowW(L"BUTTON",L"Applica formato a tutti / Apply format to all",WS_CHILD|WS_VISIBLE,240,334,230,32,wnd,(HMENU)ID_APPLY_FORMAT,0,0);}
...
void loadSelected() {
    ensureIdentityControls();ensureBulkControl(); int i=selectedService;
...
LRESULT CALLBACK proc(HWND h,UINT m,WPARAM w,LPARAM l){
 ...
 case WM_COMMAND:{int id=LOWORD(w),code=HIWORD(w);if(code==EN_CHANGE&&(id==ID_SCIDS||id==ID_SUBCH)){int i=selectedService;if(i>=0&&(size_t)i<services.size()){std::wstring v=text((HWND)l);if(id==ID_SCIDS){services[(size_t)i].shortLabel=v.substr(0,8);}else{wchar_t*end=nullptr;unsigned long n=wcstoul(v.c_str(),&end,10);if(end&&*end==0&&n<=63)services[(size_t)i].subch=(unsigned)n;}}}if(code==EN_CHANGE&&id==ID_BITRATE&&selectedService>=0&&(size_t)selectedService<services.size()){std::wstring v=text(editRate);wchar_t*end=nullptr;unsigned long n=wcstoul(v.c_str(),&end,10);if(end&&end!=v.c_str()&&*end==0){services[(size_t)selectedService].bitrate=(unsigned)n;int keep=selectedService;populateList();selectedService=keep;ListView_SetItemState(list,keep,LVIS_SELECTED,LVIS_SELECTED);status();}}if((code==EN_KILLFOCUS&&(id==ID_SERVICE||id==ID_SID||id==ID_SOURCE_VALUE||id==ID_BITRATE||id==ID_DLS||id==ID_SCIDS||id==ID_SUBCH))||(code==CBN_SELCHANGE&&(id==ID_SOURCE||id==ID_CODEC||id==ID_SAMPLING||id==ID_EEP)))saveSelected(false);switch(id){
  case ID_BROWSE:browseAudio();break;case ID_LANG:saveSelected();italian=!italian;refreshLanguage();break;case ID_ADD:addClonedService();break;case ID_APPLY_FORMAT:applyFormatToAll();break;
