...
namespace {
constexpr const wchar_t* APP_VERSION=L"1.0.10";
...
int WINAPI wWinMain(HINSTANCE i,HINSTANCE, PWSTR cmd,int){instance=i;if(cmd&&wcsncmp(cmd,L"--config",8)==0){const wchar_t*p=cmd+8;while(*p==L' ')++p;if(*p==L'\"'){++p;const wchar_t*e=wcschr(p,L'\"');startupConfig.assign(p,e?e:p+wcslen(p));}else startupConfig=p;}WNDCLASSW c{};c.hInstance=i;c.lpszClassName=L"WolfDAB";c.lpfnWndProc=proc;c.hCursor=LoadCursor(nullptr,IDC_ARROW);c.hIcon=LoadIconW(i,MAKEINTRESOURCEW(101));c.hbrBackground=(HBRUSH)(COLOR_WINDOW+1);if(!RegisterClassW(&c)&&GetLastError()!=ERROR_CLASS_ALREADY_EXISTS)return 1;constexpr DWORD style=WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX|WS_VISIBLE;HWND h=CreateWindowW(c.lpszClassName,L"WolfDAB 1.0.10",style,80,35,1160,700,nullptr,nullptr,i,nullptr);if(!h)return 2;MSG m;while(GetMessageW(&m,nullptr,0,0)){TranslateMessage(&m);DispatchMessageW(&m);}return 0;}
