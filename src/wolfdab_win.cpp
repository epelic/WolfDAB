...
void refreshLanguage(){ SetWindowTextW(wnd,tr(L"WolfDAB — Controllo multiplex DAB/DAB+",L"WolfDAB — DAB/DAB+ multiplex control"));SetWindowTextW(ampCheck,tr(L"Amplificatore ON",L"Amplifier ON"));if(applyFormatButton)SetWindowTextW(applyFormatButton,tr(L"Applica formato a tutti",L"Apply format to all")); status(); }
