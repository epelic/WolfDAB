...
namespace {
constexpr int ID_LANG=100, ID_ADD=101, ID_REMOVE=102, ID_START=103, ID_STOP=104, ID_BROWSE=105;
...
struct Service { std::wstring label=L"Nuovo servizio"; std::wstring shortLabel=L"Nuovo"; std::wstring sid=L"0xE001"; unsigned scids=0; unsigned subch=1; std::wstring source=L""; unsigned bitrate=72; unsigned codec=1; unsigned sampling=48000; dab_eep_profile_t eep=DAB_EEP_3A; std::wstring dls=L"";
    Service()=default;
...
HWND wnd, list, statusbar, editService, editSid, editScids, editSubch, comboSource, editSource, editRate, comboCodec, comboSampling, comboEep, editDls, editEnsemble, editEid, editEcc, comboChannel, editGain, ampCheck;
HINSTANCE instance;
...
void loadSelected() {
    ensureIdentityControls(); int i=selectedService;
    SendMessageW(editScids,EM_SETLIMITTEXT,8,0);
    if(i<0 || (size_t)i>=services.size())return; auto&s=services[i];
    setText(editService,s.label);setText(editSid,s.sid);setText(editScids,s.shortLabel);setText(editSubch,std::to_wstring(s.subch));setText(editRate,std::to_wstring(s.bitrate));setText(editDls,s.dls);
...
void saveSelected(bool refresh=true) {
    int i=selectedService;std::wstring shownSid=text(editSid);for(size_t k=0;k<services.size();++k)if(services[k].sid==shownSid){i=(int)k;break;}
    if(i<0 || (size_t)i>=services.size())return;selectedService=i;auto&s=services[i];s.label=text(editService);s.sid=shownSid;s.dls=text(editDls);
...
void refreshLanguage(){ SetWindowTextW(wnd,tr(L"WolfDAB — Controllo multiplex DAB/DAB+",L"WolfDAB — DAB/DAB+ multiplex control"));SetWindowTextW(ampCheck,tr(L"Amplificatore ON",L"Amplifier ON")); status(); }
...
bool loadConfig(const std::wstring& path) {
    ...
    services=std::move(next); selectedService=0; populateList(); ListView_SetItemState(list,0,LVIS_SELECTED,LVIS_SELECTED); loadSelected(); return true;
}
...
void info(){ MessageBoxW(wnd,L"WolfDAB\nDAB/DAB+ multiplex controller for HackRF One\n\nNative Windows x64 • Mode I • 2.048 MS/s\n\nTransmit only where authorised.",L"WolfDAB",MB_OK|MB_ICONINFORMATION); }
LRESULT CALLBACK proc(HWND h,UINT m,WPARAM w,LPARAM l){
 switch(m){
 case WM_CREATE:wnd=h;windowBrush=CreateSolidBrush(RGB(248,249,251));accentBrush=CreateSolidBrush(RGB(158,235,55));{BOOL dark=FALSE;DwmSetWindowAttribute(h,20,&dark,sizeof(dark));DWORD corner=2;DwmSetWindowAttribute(h,33,&corner,sizeof(corner));}addMenus();createUi();if(!startupConfig.empty()){configPath=startupConfig;loadConfig(configPath);}EnableWindow(GetDlgItem(h,ID_STOP),FALSE);return 0;
...
int WINAPI wWinMain(HINSTANCE i,HINSTANCE, PWSTR cmd,int){instance=i;if(cmd&&wcsncmp(cmd,L"--config",8)==0){const wchar_t*p=cmd+8;while(*p==L' ')++p;if(*p==L'\"'){++p;const wchar_t*e=wcschr(p,L'\"');startupConfig.assign(p,e?e:p+wcslen(p));}else startupConfig=p;}WNDCLASSW c{};c.hInstance=i;c.lpszClassName=L"WolfDAB";c.lpfnWndProc=proc;c.hCursor=LoadCursor(nullptr,IDC_ARROW);c.hIcon=LoadIconW(i,MAKEINTRESOURCEW(101));c.hbrBackground=(HBRUSH)(COLOR_WINDOW+1);if(!RegisterClassW(&c)&&GetLastError()!=ERROR_CLASS_ALREADY_EXISTS)return 1;constexpr DWORD style=WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX|WS_VISIBLE;HWND h=CreateWindowW(c.lpszClassName,L"WolfDAB",style,80,35,1160,700,nullptr,nullptr,i,nullptr);if(!h)return 2;MSG m;while(GetMessageW(&m,nullptr,0,0)){TranslateMessage(&m);DispatchMessageW(&m);}return 0;}
